...
/** Generate a grid-like or ChanServ-style output from a given input.
 *
 *  @code
 *  CTable table;
 *  table.AddColumn("a");
...
+-------+-------+@endverbatim
 *
 *  ChanServ-style output can be generated like so (only supports two columns!):
 *  @code
 *  while (table.GetLine(idx++, tmp, true)) {
 *      // Output tmp somehow
 *  }
 *  @endcode
...
class CTable : protected std::vector<std::vector<CString>> {
  public:
    typedef enum { GridTable, ChanServ } EType;
    CTable() {}
    virtual ~CTable() {}
     ...
     *  @param eType (optional) Display type.
     *  @return True unless uIdx is past the end of the table.
     */
    bool GetLine(unsigned int uIdx, CString& sLine, EType eType = GridTable) const;
